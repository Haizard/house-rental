...
export function ReportForm({ targetType, targetId }: { targetType: "LISTING" | "AGENT" | "USER" | "MESSAGE"; targetId: string }) {
  ...
  return <form className="glass-surface mt-3 space-y-4 p-4" onSubmit={submit}><div className="flex items-center justify-between"><h2 className="font-semibold">Report this listing</h2><button className="flex size-10 items-center justify-center rounded-full" type="button" onClick={() => setOpen(false)} aria-label="Close report form"><X size={18} aria-hidden="true" /></button></div><label className="block text-sm font-medium">Reason    <select className="glass-search mt-2 w-full px-3 py-3 outline-none" value={reason} onChange={(event) => setReason(event.target.value)}>{reasons.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label className="block text-sm font-medium">Details<textarea className="glass-search mt-2 min-h-24 w-full resize-y px-3 py-3 outline-none" value={description} onChange={(event) => setDescription(event.target.value)} maxLength={2000} placeholder="Tell us what needs attention." /></label>{status && <p className="text-sm text-[var(--text-secondary)]" role="status">{status}</p>}<button className="button button-primary w-full" disabled={busy} type="submit">{busy ? "Sending..." : "Send report"}</button></form>;
}
