...
export function ReportResolution({ reportId }: { reportId: string }) {
  ...
  return <div className="space-y-2"><label className="sr-only" htmlFor={`resolution-${reportId}`}>Resolution</label><input className="glass-search w-full px-3 py-2 text-sm outline-none" id={`resolution-${reportId}`} value={resolution} onChange={(event) => setResolution(event.target.value)} placeholder="Add resolution" maxLength={2000} />{error && <p className="text-xs text-red-600" role="alert">{error}</p>}<div className="flex gap-2"><button className="button button-primary h-8 flex-1 px-3 text-sm" disabled={busy} type="button" onClick={() => resolve("RESOLVED")}><Check size={16} aria-hidden="true" />Resolve</button><button className="button button-glass h-8 flex-1 px-3 text-sm" disabled={busy} type="button" onClick={() => resolve("DISMISSED")}><X size={16} aria-hidden="true" />Dismiss</button></div></div>;
}
