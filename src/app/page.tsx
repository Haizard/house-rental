...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen px-4 pb-12 pt-4 sm:px-8 lg:px-12">
      <TopBar />
      <nav className="glass-nav mx-auto flex max-w-7xl items-center justify-between overflow-hidden px-4 py-3 sm:px-5">
        <a className="flex shrink-0 items-center gap-2 font-semibold text-[var(--text-primary)]" href="#top">
          ...
          <span className="truncate">Nyumba Nearby</span>
        </a>
      ...
      <section id="top" className="mx-auto grid max-w-7xl items-end gap-8 overflow-hidden pb-10 pt-14 lg:grid-cols-[1.1fr_.9fr] lg:pt-24">
        <div className="overflow-hidden">
          <p className="eyebrow">Student housing in Arusha</p>
          <h1 className="mt-3 max-w-3xl text-[32px] font-bold leading-[1.08] text-[var(--text-primary)] sm:text-[40px] lg:text-[56px]">A better way to find your next place.</h1>
          <p className="mt-5 max-w-xl text-[15px] leading-7 text-[var(--text-secondary)] sm:text-[17px]">Explore available rooms and homes around Njiro, then talk directly with a trusted local agent when one feels right.</p>
        ...
        <aside className="glass-surface relative overflow-hidden p-5 sm:p-6">
          <div className="absolute -right-10 -top-8 size-40 rounded-full bg-sky-200/50 blur-3xl" />
          <p className="relative text-sm font-medium text-[var(--accent)]">Made for real student moves</p>
          <div className="relative mt-7 grid grid-cols-2 gap-3">
            <div className="stat-tile overflow-hidden"><strong>150k</strong><span>From TZS / month</span></div>
            <div className="stat-tile overflow-hidden"><strong>24h</strong><span>Typical agent reply</span></div>
            <div className="col-span-2 flex items-center gap-3 overflow-hidden rounded-[14px] bg-white/45 p-3">
              <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-emerald-600">
                <ShieldCheck size={20} aria-hidden="true" />
              </span>
              <p className="min-w-0 text-sm leading-5 text-[var(--text-secondary)]">
                <b className="text-[var(--text-primary)]">Agent-led, verified listings.</b><br />
      ...
      <section id="listings" className="mx-auto max-w-7xl overflow-hidden">
        <div className="mb-5 flex items-end justify-between gap-4">
          <div>
            <p className="eyebrow">Available now</p>
            <h2 className="mt-1 text-2xl font-bold text-[var(--text-primary)]">Homes around Njiro</h2>
          </div>
          <a className="hidden shrink-0 items-center gap-1 text-sm font-medium text-[var(--accent)] sm:flex" href="/search">
      ...
      <section id="agents" className="mx-auto mt-14 max-w-7xl overflow-hidden glass-surface grid gap-6 p-6 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center">
        <div className="overflow-hidden">
          <p className="eyebrow">For local agents</p>
          <h2 className="mt-2 text-xl font-bold text-[var(--text-primary)] sm:text-2xl">Bring your available homes to students who are ready to move.</h2>
          <p className="mt-2 max-w-2xl text-[14px] leading-6 text-[var(--text-secondary)] sm:text-[15px]">Manage listings, receive qualified leads, and keep every conversation in one place.</p>
        </div>
        <a className="button button-glass shrink-0 px-5" href="/auth/agent-signup">
