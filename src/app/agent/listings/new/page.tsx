    ...
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mb-8 flex items-center gap-1.5 overflow-x-auto pb-2 scrollbar-hide sm:justify-center sm:gap-2">
        {STEPS.map(({ label, icon: Icon }, i) => (
          <button
            key={label}
            className={`inline-flex shrink-0 items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
              i === step
            ...
            <div key={category}>
              ...
              <div className="flex flex-wrap gap-2">
                {items.map(({ slug, label }) => (
                  <button
                    key={slug}
                    className={`inline-flex items-center justify-center rounded-lg border px-3 py-1.5 text-sm font-medium transition-all ${
