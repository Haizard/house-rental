    ...
    <main className="min-h-screen px-4 py-4 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.35fr_.65fr]">
          ...
          <aside className="glass-surface h-fit p-5 sm:p-6"><p className="price mt-0 text-3xl">TZS {price}<span className="text-sm font-normal text-[var(--text-secondary)]"> / month</span></p><p className="mt-2 text-[15px] text-[var(--text-secondary)]">{listing.type} in {listing.area}</p>{listing.verified && <p className="mt-5 flex items-center gap-2 text-sm font-medium text-emerald-700"><BadgeCheck size={18} aria-hidden="true" />Verified listing</p>}<Link className="mt-5 block text-sm font-medium text-[var(--accent)]" href={`/agents/${listing.agentId}`}>View agent profile</Link><div className="mt-6 space-y-3"><button className="button button-primary w-full" type="button"><MessageCircle size={19} aria-hidden="true" />Chat with agent</button><button className="button button-glass w-full" type="button">Request a viewing</button></div><div className="mt-6 border-t border-black/[.07] pt-5"><p className="flex items-start gap-3 text-sm leading-5 text-[var(--text-secondary)]"><ShieldCheck className="mt-0.5 shrink-0 text-[var(--accent)]" size={19} aria-hidden="true" />You choose the agent before opening a conversation. No fee is charged to students.</p></div></aside>
        </div>
      </div>
    </main>
  );
