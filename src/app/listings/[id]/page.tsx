          ...
          <section className="overflow-hidden rounded-[22px] border border-white/70 bg-white/45 shadow-[var(--glass-shadow)]">
            ...
            {listing.images && listing.images.length > 1 && (
              <div className="grid grid-cols-2 gap-1 p-1 sm:grid-cols-3">
                {listing.images.slice(1, 7).map((img) => (
                  <div className="relative aspect-[4/3] overflow-hidden rounded-lg" key={img.id}>
                    <Image src={img.url} alt="" fill className="object-cover" sizes="(max-width: 767px) 50vw, 22vw" />
                  </div>
                ))}
              </div>
            )}
