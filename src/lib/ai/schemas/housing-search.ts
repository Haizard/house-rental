...
export const housingSearchSchema = z.object({
  ...
  area: z.string().optional().nullable().transform(v => v ?? undefined).describe("Area or neighborhood name"),
  ...
  minPrice: z.number().int().positive().optional().nullable().transform(v => v ?? undefined).describe("Minimum monthly rent in TZS"),
  /** Maximum monthly rent in TZS */
  maxPrice: z.number().int().positive().optional().nullable().transform(v => v ?? undefined).describe("Maximum monthly rent in TZS"),
  /** Whether the room must be self-contained (own bathroom) */
  selfContained: z.boolean().optional().nullable().transform(v => v ?? undefined).describe("Whether the room must be self-contained"),
  /** Whether Wi-Fi/internet is required */
  internet: z.boolean().optional().nullable().transform(v => v ?? undefined).describe("Whether internet/Wi-Fi is required"),
